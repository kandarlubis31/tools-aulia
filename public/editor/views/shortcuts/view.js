...
export const ShortcutsManager = shadow_view((use) => () => {
    ...
    const [_, setPressedKeys, getPressedKeys] = use.state(new Set());
    const [conflict, setConflict] = use.state(null);
    // Render the list of shortcuts
    const renderShortcutsList = () => manager.listShortcuts().map(({ shortcut, actionType }) => html `
				<tr>
					<td>${actionType}</td>
					<td>
						<input
							type="text"
							.value=${shortcut}
							@keyup=${(e) => onPointerUpInput(e, actionType)}
