...
export const CollaborationManager = shadow_view((use) => () => {
    ...
    const [sessionError, setSessionError] = use.state("");
    ...
    const renderHostModal = () => {
        return renderModal(html `
            ...
            ? [...collaboration.connectedClients.entries()].map(([id, client]) => html `
						<div class=peer>
    ...
    const renderNoSessionModal = () => {
        return renderModal(html `
			...
			<div>
				<h3>Join Session</h3>
				<input class="code-input" placeholder="Enter Invite Code" @input=${(e) => setInviteID(e.target.value)}>
