...
> **Last updated:** September 2, 2026 (Optional chaining audit · alert() → showToast() · error boundary auto-dismiss fix)
...
17. **Optional chaining on `window.showToast`** (Sep 2, 2026) — Semua `window.showToast(...)` WAJIB pakai `?.()`: `window.showToast?.(msg, type)`. Tanpa optional chaining, kalau `showToast` belum di-definisi (race condition waktu page load / SPA navigation), throw `TypeError` → ketangkep error boundary → nampilin toast “Terjadi error” yang misleading. **Aturan:** `window.showToast?.(` SELALU, kecuali ada guard `if (window.showToast)` sebelumnya (pattern lama, toleransi tapi tidak preferred).
18. **`alert()` dilarang** (Sep 2, 2026) — Semua `alert()` native browser diganti `window.showToast?.(_tToast(msg), 'error')`. `alert()` blocking, gak konsisten sama toast system, dan gak di-translate pas mode English. Guardrail: grep `alert(` di prebuild script bisa ditambah.
19. **Error boundary auto-dismiss 30s** (Sep 2, 2026) — Toast error boundary di `BaseLayout.astro` auto-dismiss dinaikin dari 15s → 30s. 15s terlalu pendek — user sering gak notice (tab di background, lagi baca), error ilang tanpa action.
20. **SW offline-fallback routing** (Aug 23, 2026) — Bug "kena 📡 Kamu Offline padahal online": navigasi dengan query (`/?cat=dev`, `?from_cat=`) MISS precache (default Workbox cuma ignore `utm_*/fbclid`) → langsung disodori `/offline/` TANPA coba network, lalu self-heal halaman itu menghapus semua cache PWA → siklus 40MB install ulang. Fix: `ignoreURLParametersMatching: [/.*/]` di workbox config (query = UI state murni) + self-heal kini hanya menghapus cache `workbox-precache*` (cache CDN/api dipertahankan). ⚠️ Jangan tambah runtimeCaching untuk `request.mode==='navigate'`: template generateSW mendaftarkan NavigationRoute(navigateFallback) SEBELUM runtime routes → route navigasi kustom pasti ke-shadow (dead code).
...
- **Error/Notification System Audit (Sep 2, 2026):** ✅ — (1) 7 file `alert()` → `showToast()` (qr-scanner, compare, booklet, random, palette, collage); (2) ~65+ `window.showToast(...)` → `window.showToast?.(...)` di 17 file (unit, bmi, age, case, BaseLayout, color, uuid, password, hash, jokes, lorem, url, my-ip, markdown, json, base64, motivation); (3) error boundary auto-dismiss 15s → 30s. Semua 62 tests pass.
