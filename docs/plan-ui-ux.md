...
| **Follow-up selesai (Aug 11)** | sisa text-matte-300 light dibereskan: word-counter (freq-empty → `text-matte-500`, nomor 10px → `text-matte-500`) · split/rotate button di toolbar gelap → `text-matte-200` (kontras ~9:1) · **+2 straggler ketemu di my-ip** (label "Mbps" & separator "·" di riwayat speed test + separator riwayat IP → `text-matte-400/500`) · sisa aman diverifikasi: dekoratif (SVG 404, tick gauge, breadcrumb separator aria-hidden) atau di surface gelap (copy-json btn) |
